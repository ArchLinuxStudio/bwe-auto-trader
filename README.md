...
- Windows / macOS / Linux 源码与构建脚本（Electron + React + TypeScript）；`0.1.10` 在 Windows x64 提供带自定义应用图标的 NSIS 安装包和便携 ZIP，并完成 afterPack、ZIP 完整解压及便携版隔离冷启动验证；macOS/Linux 仍需对应系统或 CI 验证
...
- 系统安全存储、审计日志、选择性系统全局通知、紧急停止；应用打开后会按已有配置自动尝试恢复连接，但每次重启仍自动锁定实盘
...
5. 首次配置时，在三个连接全绿后开启监听。以后打开应用时，Telegram、ChatGPT 和 OKX 会分别按各自已有配置独立尝试连接；只有三项配置都存在且三项连接都完整成功，程序才自动开启 Telegram 监听。观察分析结果无误后，仍需在本次进程中输入“确认实盘”才能解锁真实下单。

保存的 ChatGPT“已配置”状态只表示启动时值得尝试恢复，不代表登录仍然有效。程序必须以 Codex app-server 返回的实际账户和读取状态为准；会话失效、模型预热失败或额度状态无法建立时，ChatGPT 保持未连接或错误状态并提示手动重试。任一服务自动连接失败不会取消另外两项的独立尝试，但也不会自动开启监听。

这里的“打开应用后自动连接”不是随 Windows/macOS/Linux 登录自动启动，也不是 headless 后台服务或无人值守模式。程序仍只在进程运行期间监听，且当前缺少自动止损、止盈和退出时平仓等无人值守保护。

任何重启、主动断开/停止监听、Telegram 登录或不可恢复错误、OKX 数据流异常、订单状态不明确或相关服务生命周期变更，都会自动锁定实盘。启动时恢复连接和自动开启监听绝不会创建、恢复或持久化“确认实盘”授权。Telegram 因网络波动疑似掉线时会立即关闭内部交易门禁；确认掉线后程序显示“实盘重连中”，每个后续健康周期继续一次有超时上限的自动重连。校验/补拉期间不会解锁或发单，断线前已进入处理链的消息授权也永久失效。只有同一监听实例完整补拉、复核登录授权并重新开放 readiness 后，原来已经人工确认的进程内实盘意图才会自动继续，而且仅对恢复后新到达的实时消息生效；启动或重连补拉消息永远不能下单。每条实时消息还会绑定其到达时的授权与连接代次，此门禁会一直复核到 OKX 真实订单 POST 前。teleproto 的一般可恢复错误只作诊断，不能单独触发状态切换。状态不明确的订单只会只读对账，永不自动重试。专用子账户持仓区会显示该子账户的全部 SWAP 持仓，不会把其他来源的仓位误称为本程序仓位。

启用“桌面通知”后，收到频道消息、Telegram 确认断线并进入自动重连、AI 开始分析频道消息，以及信号通过最终检查并进入下单操作时，会由 Electron 主进程发送系统全局通知。普通状态、跳过和错误提示继续保留在程序内。系统通知正文不会显示频道原文、消息编号、币种、方向、金额或订单号；点击通知会尝试恢复主窗口。操作系统的通知权限、专注助手或平台能力可能抑制显示，通知结果从不参与交易授权或状态判断。
...
如果界面提示“订单结果未知”或连接在下单期间中断，不要再次点击、不要更换 API 凭据来尝试清除状态。程序会在真实订单 POST 前原子写入脱敏的 mutation journal，并在 ACK、部分成交、未知或终态时更新；崩溃或重启不会清除仍未取得终态的互锁。重启后的 OKX 自动连接与用户手动连接使用同一套账户身份检查和只读恢复：精确匹配的终态可以解除记录，但新 client 即使多次“未找到”也会继续锁定，且任何恢复结果都不会自动解锁实盘。请同时在 OKX 官方客户端按交易对、客户订单号和订单号核对，不要把重启或自动连接当作交易所侧核验的替代品。
...
macOS 和 Linux 安装包应分别在对应系统或 CI runner 上生成。当前 `0.1.10` Windows 安装包和主程序未配置发布者代码签名，Windows SmartScreen 可能警告；长期正式分发建议配置 Windows 代码签名，macOS 则需 Developer ID 签名与公证。
