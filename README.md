...
- Windows / macOS / Linux 源码与构建脚本（Electron + React + TypeScript）；`0.1.9` 在 Windows x64 提供带自定义应用图标的 NSIS 安装包和便携 ZIP，并完成 afterPack、ZIP 完整解压及便携版隔离冷启动验证；macOS/Linux 仍需对应系统或 CI 验证
...
任何重启、主动断开/停止监听、Telegram 登录或不可恢复错误、OKX 数据流异常、订单状态不明确或相关服务生命周期变更，都会自动锁定实盘。Telegram 因网络波动疑似掉线时会立即关闭内部交易门禁；确认掉线后程序显示“实盘重连中”，每个后续健康周期继续一次有超时上限的自动重连。校验/补拉期间不会解锁或发单，断线前已进入处理链的消息授权也永久失效。只有同一监听实例完整补拉、复核登录授权并重新开放 readiness 后，原来已经人工确认的进程内实盘意图才会自动继续，而且仅对恢复后新到达的实时消息生效；启动或重连补拉消息永远不能下单。每条实时消息还会绑定其到达时的授权与连接代次，此门禁会一直复核到 OKX 真实订单 POST 前。teleproto 的一般可恢复错误只作诊断，不能单独触发状态切换。状态不明确的订单只会只读对账，永不自动重试。专用子账户持仓区会显示该子账户的全部 SWAP 持仓，不会把其他来源的仓位误称为本程序仓位。
...
macOS 和 Linux 安装包应分别在对应系统或 CI runner 上生成。当前 `0.1.9` Windows 安装包和主程序未配置发布者代码签名，Windows SmartScreen 可能警告；长期正式分发建议配置 Windows 代码签名，macOS 则需 Developer ID 签名与公证。
